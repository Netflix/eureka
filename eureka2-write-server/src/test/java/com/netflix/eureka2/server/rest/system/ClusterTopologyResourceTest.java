...
public class ClusterTopologyResourceTest {
    ...
    public void setUp() throws Exception {
        ...
        when(registry.forInterest(Interests.forFullRegistry())).thenReturn(Observable.just(
                new ChangeNotification<InstanceInfo>(Kind.Add, WEB_INSTANCE),
                new ChangeNotification<InstanceInfo>(Kind.Add, EUREKA_WRITE_INSTANCE),
                new ChangeNotification<InstanceInfo>(Kind.Add, EUREKA_READ_INSTANCE),
                ChangeNotification.<InstanceInfo>bufferSentinel()
        ).concatWith(Observable.<ChangeNotification<InstanceInfo>>never()));
    }
