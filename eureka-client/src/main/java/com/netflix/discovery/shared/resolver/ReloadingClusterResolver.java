...
public class ReloadingClusterResolver<T extends EurekaEndpoint> implements ClusterResolver<T> {
    ...
    public ReloadingClusterResolver(final ClusterResolverFactory<T> factory, final long reloadIntervalMs) {
        ...
        if (logger.isInfoEnabled()) {
            logger.info("Initiated with delegate resolver of type {}; next reload in {}[sec]. Loaded endpoints={}", delegateRef.get().getClass(), currentReloadIntervalMs / 1000, clusterEndpoints);
        }
        try {
    ...
    public List<T> getClusterEndpoints() {
        ...
        if (expiryTime <= System.currentTimeMillis()) {
            try {
                ...
                logger.warn("Cluster resolve error; keeping the current Eureka endpoints; next reload in " + "{}[sec]", currentReloadIntervalMs / 1000, e);
            }
        }
    ...
    @Monitor(name = METRIC_RESOLVER_PREFIX + "lastReloadTimestamp", description = "How much time has passed from last successful cluster configuration resolve", type = DataSourceType.GAUGE)
