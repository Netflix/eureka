...
public class EurekaClientLifecycleServerFailureTest {
    ...
    public static void setupClass() throws IOException {
        ...
        when(requestHandler.getApplications()).thenReturn(anEurekaHttpResponse(500, APPLICATIONS).type(MediaType.APPLICATION_JSON_TYPE).build());
        when(requestHandler.getDelta()).thenReturn(anEurekaHttpResponse(500, APPLICATIONS_DELTA).type(MediaType.APPLICATION_JSON_TYPE).build());
    }
    ...
    public void testEnforceFetchRegistryAtInitPrimaryFailureAndBackupSuccess() {
        Injector injector = LifecycleInjector.builder().withModules(new AbstractModule() {
            ...
            protected void configure() {
                ...
                bind(AbstractDiscoveryClientOptionalArgs.class).to(Jersey1DiscoveryClientOptionalArgs.class).in(Scopes.SINGLETON);
                bind(EndpointRandomizer.class).toInstance(ResolverUtils::randomize);
                // returns empty list on registry fetch
                bind(BackupRegistry.class).toInstance(new MockBackupRegistry());
            }
        }).build().createInjector();
        LifecycleManager lifecycleManager = injector.getInstance(LifecycleManager.class);
        try {
