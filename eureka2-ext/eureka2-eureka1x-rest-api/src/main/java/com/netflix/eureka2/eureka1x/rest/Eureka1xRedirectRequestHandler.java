 ...
 * @author Tomasz Bak
 */
public class Eureka1xRedirectRequestHandler extends AbstractEureka1xRequestHandler {

    private static final Logger logger = LoggerFactory.getLogger(Eureka1xRedirectRequestHandler.class);

    private static final ServiceSelector HTTP_SERVICE_SELECTOR = ServiceSelector.selectBy()
    ...
    @Inject
    public Eureka1xRedirectRequestHandler(ExtensionContext context,
    ...
    private Observable<Void> redirectTo(InstanceInfo readServerInfo,
        ...
        ServiceEndpoint serviceEndpoint = HTTP_SERVICE_SELECTOR.returnServiceEndpoint(readServerInfo);

        StringBuilder redirectBuilder = new StringBuilder("http://")
                .append(serviceEndpoint.getAddress().getIpAddress())
                .append(':')
