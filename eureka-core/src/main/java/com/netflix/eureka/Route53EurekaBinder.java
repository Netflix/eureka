 ...
 */
public class Route53EurekaBinder implements EurakaBinder {
    private static final Logger logger = LoggerFactory
            .getLogger(Route53EurekaBinder.class);
    public static final String NULL_DOMAIN = "null";

    private final EurekaServerConfig eurekaServerConfig = EurekaServerConfigurationManager
    ...
    private final AmazonRoute53Client amazonRoute53Client = getAmazonRoute53Client();

    public Route53EurekaBinder() {
        this(getRegistrationHostnameFromAmazonDataCenterInfo());
    }

    /**
     * @param registrationHostname the hostname to register under the Route53 CNAME
     */
    public Route53EurekaBinder(String registrationHostname) {
        this.registrationHostname = registrationHostname;
        this.timer = new Timer("Eureka-Route53Binder", true);
    }

    public static String getRegistrationHostnameFromAmazonDataCenterInfo() {
    ...
    public void bind() throws InterruptedException {
        ...
        timer.schedule(
            ...
            eurekaServerConfig.getRoute53BindingRetryIntervalMs(),
            eurekaServerConfig.getRoute53BindingRetryIntervalMs());
    }
        ...
        for(String domain : domains) {
            ...
            if (rrs != null) {
                if (rrs.getResourceRecordSet() == null) {
                    ...
                    resourceRecordSet.setTTL(eurekaServerConfig.getRoute53DomainTTL());
    ...
    private boolean executeChangeWithRetry(Change change, HostedZone hostedZone) throws InterruptedException {
        ...
        for (int i = 0; i < EurekaServerConfigurationManager.getInstance()
                .getConfiguration().getRoute53BindRebindRetries(); i++) {
            try {
